    ...
    $(".login_form_con").submit(function (e) {
        ...
        $.post('/user/login', {
            ...
            if (data.result == 1) {
                ...
                //如果当前地址是详细页面，则处理“关注”按钮、“收藏”按钮
                if(/\d+/.test(location.pathname)){//当前是详细页面
                    //请求服务器，查看是否收藏，是否关注
                    //返回数据后控制相应div的显示与隐藏
                }
    ...
    $('#logout').click(function () {
        $.post('/user/logout', {'csrf_token': $('#csrf_token').val()}, function (data) {
            if (data.result == 1) {
                ...
                //对于user.html退出时需要返回首页
                if (location.pathname == '/user/') {
                    location.href = '/';
                } else {//对于index、detail只是隐藏用户div，显示登录注册div
                    $('.user_btns').show();
                    $('.user_login').hide();
                    if(/\d+/.test(location.pathname)){//详细页
                        $('.focus').show();
                        $('.focused').hide();
                        $('.collection').show();
                        $('.collected').hide();
